...
このカタログを別プロジェクト（ProjectX）で使用する場合：

```
ProjectX/
├── catalog/          ← このリポジトリの catalog/ をコピー（ベンダリング）
├── docs/
│   └── CATALOG_VERSION.md  ← コミットIDを記録
├── specs/
└── manifests/
```
...
詳細: `catalog/consumer-example/README.md` 参照
...
## 7. レッドフラグ・トリガー（確認が必要な状況）

以下の状況では、**実装を進めずにユーザーに確認**すること：

```
🚩 パターンYAMLに該当しない要求がある
🚩 2つ以上のパターンが候補になり判断できない
🚩 SPECの characteristics に未定義の特性がある
🚩 既存コードとカタログの矛盾を発見した
🚩 ドメインルールの解釈に自信がない
```

**判断基準**: 迷ったら聞く。間違った実装より確認の方がコストが低い。
