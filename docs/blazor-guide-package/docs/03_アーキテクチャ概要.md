...
#### **Monolithic Vertical Slice Architecture (モノリシックVSA)**

**🎯 このプロジェクトの特徴:**

このプロジェクトは、**機能(Feature)を最上位とした垂直スライスアーキテクチャ**をモノリシック構成で実装しています。
従来のレイヤー分離アーキテクチャとは異なり、**単一Applicationプロジェクトに全機能を集約**し、機能追加時の変更範囲を最小化します。
...
**重要:**
- **モノリシック構成**: 単一Applicationプロジェクトに全機能を集約（YAGNI原則）
- **各機能（CreateProduct, GetProducts等）はCommand/Handler/UIを持つ**
- **ドメインモデルはBC別にDomainプロジェクトで分離**（ProductCatalog, PurchaseManagement）
- **共通コードはSharedプロジェクトで一元管理**（Kernel, Domain, Application, Infrastructure）
...
| 観点 | Clean Architecture | モノリシックVSA (このプロジェクト) |
|-----|-------------------|---------------------|
| 最上位の分割 | レイヤー(Application/Domain/Infrastructure/UI) | 機能(Application/Features/{FeatureName}) |
| プロジェクト構成 | ProductCatalog.Application<br>ProductCatalog.Domain<br>ProductCatalog.Infrastructure<br>ProductCatalog.UI | Application (モノリシック)<br>Domain/ProductCatalog<br>Domain/PurchaseManagement<br>Shared/* |
| 機能追加時の変更 | 複数の層プロジェクトを変更 | 1つの機能フォルダ内のみ（Application/Features/{FeatureName}） |
| 依存方向 | 層間の依存方向を厳密に管理 | Application → Domain → Shared/Kernel<br>機能間の直接依存は禁止 |
| 適用場面 | 層の責務を厳密に分離したい場合<br>複数チームで層別開発 | 機能の独立性を重視する場合<br>素早い機能追加が必要な場合<br>YAGNI原則に従う場合 |
