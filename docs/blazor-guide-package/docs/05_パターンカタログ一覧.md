...
## 🏢 実装例: PurchaseManagement BC（購買管理）

ProductCatalog BCに加えて、**PurchaseManagement BC**には複雑なビジネスプロセスの実装例が含まれています。

### 承認ワークフローパターン

複数ステップの業務フローを実装する場合の参考例：

| 機能 | 実装場所 | 用途 |
|-----|---------|------|
| **SubmitPurchaseRequest** | `src/PurchaseManagement/Features/SubmitPurchaseRequest/` | 購買申請の新規作成・承認フロー開始 |
| **ApprovePurchaseRequest** | `src/PurchaseManagement/Features/ApprovePurchaseRequest/` | 承認処理・次ステップへの遷移 |
| **RejectPurchaseRequest** | `src/PurchaseManagement/Features/RejectPurchaseRequest/` | 却下処理・差し戻し |
| **CancelPurchaseRequest** | `src/PurchaseManagement/Features/CancelPurchaseRequest/` | 申請のキャンセル・状態管理 |

**特徴:**
- 状態遷移の管理（Draft → Submitted → Approved → Rejected）
- ドメインイベントによる承認履歴の記録
- ビジネスルールの検証（承認権限、金額上限など）
...
| 機能 | 実装場所 | 用途 |
|-----|---------|------|
| **GetPendingApprovals** | `src/PurchaseManagement/Features/GetPendingApprovals/` | 承認待ち一覧（ロールベースフィルタリング） |
| **GetDashboardStatistics** | `src/PurchaseManagement/Features/GetDashboardStatistics/` | ダッシュボード用集計データ |
| **GetPurchaseRequestById** | `src/PurchaseManagement/Features/GetPurchaseRequestById/` | 単一申請の詳細取得 |
| **GetPurchaseRequests** | `src/PurchaseManagement/Features/GetPurchaseRequests/` | 申請一覧（検索・フィルタリング） |

**特徴:**
- Dapperによる高速集計クエリ
- マルチテナント対応のデータ分離
- キャッシュ戦略（承認待ちデータは短期キャッシュ）
...
| 機能 | 実装場所 | 用途 |
|-----|---------|------|
| **UploadAttachment** | `src/PurchaseManagement/Features/UploadAttachment/` | ファイルアップロード・購買申請への関連付け |
...
### いつPurchaseManagementを参考にすべきか

以下のようなシナリオでは、PurchaseManagement BCの実装が参考になります：

- ✅ **複数ステップのワークフロー** - 状態遷移が複雑な業務プロセス（承認フロー、申請処理など）
- ✅ **ロールベース認可** - ユーザーの権限に応じた処理分岐
- ✅ **ダッシュボード・レポート** - 複雑な集計クエリ、KPI表示
- ✅ **ファイル管理** - ドキュメント添付、画像アップロード
- ✅ **マルチテナント対応** - テナントごとのデータ分離

**関連ドメインモデル:**
- `src/PurchaseManagement/Shared/Domain/PurchaseRequest.cs` - 購買申請集約ルート
- `src/PurchaseManagement/Shared/Domain/ApprovalFlow.cs` - 承認フロー管理
