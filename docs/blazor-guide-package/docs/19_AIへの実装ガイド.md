...
├── UpdateProductCommandHandler.cs      ← ビジネスロジックの流れを学ぶ
└── UI/
    └── Api/Dtos/                ← DTOの定義を学ぶ

src/Domain/ProductCatalog/
└── Products/
    └── Product.cs               ← ビジネスルールを学ぶ（集約ルート）

src/Application/Infrastructure/
└── Persistence/
    └── Repositories/            ← 永続化パターンを学ぶ
```

**重要な変更:**
- **モノリシック構成**: 全機能は `src/Application/Features/{FeatureName}` に配置
- **ドメインモデル分離**: `src/Domain/{BoundedContext}` に配置（ProductCatalog, PurchaseManagement）
- **共通インフラ**: `src/Application/Infrastructure` または `src/Shared/Infrastructure` に配置
...
```
参照: src/Application/Features/UpdateProduct/
  ↓ パターンを抽出
  ↓
適用: src/Application/Features/UpdateOrder/
  ...
  │   {
  │       // Domain層から取得（src/Domain/OrderManagement/Orders/Order.cs）
...
注: ドメインモデルは src/Domain/OrderManagement/Orders/Order.cs に配置
```
