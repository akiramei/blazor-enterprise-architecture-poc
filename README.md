...
### Step 1: プロジェクト構造パターンを読む

> **⚠️ 最重要**: [catalog/scaffolds/project-structure.yaml](catalog/scaffolds/project-structure.yaml)

このファイルには以下が定義されています：
- 5プロジェクト構成（Kernel, Domain.{BC}, Shared.Application, Shared.Infrastructure, Application）
- 依存関係ルール
- UI配置ルール
- 命名規則
- 検証チェックリスト

### Step 2: 新規プロジェクトに CLAUDE.md を作成
...
1. `catalog/scaffolds/project-structure.yaml` でプロジェクト構造を確認
2. `catalog/AI_USAGE_GUIDE.md` を読み込む
3. `catalog/index.json` でパターンを検索
...
| `catalog/scaffolds/project-structure.yaml` | ✅ 必須 | 標準プロジェクト構造 |
